...
const toggleLike = async (req, res) => {
    try {
        ...
        const data = await likesService.toggleLike(likeableType, likeableId, req.user.id);
...
const checkUserLike = async (req, res) => {
    try {
        ...
        const data = await likesService.checkUserLike(likeableType, likeableId, req.user.id);
        response.success(res, 200, data);
    } catch (error) {
        response.error(res, 500, error.message);
    }
