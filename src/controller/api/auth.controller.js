const authService = require("@/service/api/auth.service");
...
const login2fa = async (req, res) => {
    try {
        ...
        const jwtServiceLocal = require("@/service/api/jwt.service");
        ...
        const jwt = require("@/service/api/jwt.service");
        const tokenData = jwt.generateAccessToken(user.id);
        const refresh_token =
            await require("@/service/api/refreshToken.service").createRefreshToken(
                user.id
            );
