        ...
        hooks: {
            beforeCreate: async (user) => {
                ...
                if (user.frist_name || user.last_name) {
                    user.full_name = `${user.frist_name || ''} ${user.last_name || ''}`.trim();
                }
            ...
            beforeUpdate: async (user) => {
                ...
                if (user.changed('frist_name') || user.changed('last_name')) {
                    user.full_name = `${user.frist_name || ''} ${user.last_name || ''}`.trim();
                }
                
                // Update username if email, first_name or last_name changed
                if (user.changed('email') || user.changed('frist_name') || user.changed('last_name')) {
                    user.username = await generateUniqueUsername(
                        user.email, 
                        User, 
                        user.id, 
