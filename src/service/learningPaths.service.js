const { LearningPath, LearningPathCourse, Course, Track, Lesson, UserLesson, User } = require("@models");
...
const list = async (currentUser = null) => {
    ...
    const enrolledCourses = currentUser
      ? (await currentUser.getCourses({ attributes: ["id"] })).map(c => c.id)
      : [];
      ...
      const coursesWithProgress = await Promise.all(
        (lp.courses || []).map(async (course) => {
          const totalLessons = await Lesson.count({
            include: [{ model: Track, as: "track", where: { course_id: course.id } }],
          });
